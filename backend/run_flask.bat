...
wt.exe -w myWindow nt -p "Command Prompt" -d "%FLASK_DIR%" --title "reject_requests.py" -- cmd /c "set FLASK_APP=view_requests.py && set FLASK_ENV=development && set FLASK_DEBUG=1 && flask run --port 5102"

wt.exe -w myWindow nt -p "Command Prompt" -d "%FLASK_DIR%" --title "status_log.py" -- cmd /c "set FLASK_APP=view_requests.py && set FLASK_ENV=development && set FLASK_DEBUG=1 && flask run --port 5003"